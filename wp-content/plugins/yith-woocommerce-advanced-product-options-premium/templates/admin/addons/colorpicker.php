	...
	<div class="col-right">
			...
			<div class="field">
				...
				yith_plugin_fw_get_field(
					array(
						...
						'alpha_enabled' => false,
						'default'       => '#',
						'value'         => $addon->get_option( 'colorpicker', $x, '#ffffff' ),
					),
					true
				);
				?>
		...
		<div class="field-wrap colorpicker-placeholder" style="<?php echo 'placeholder' !== $colorpicker_show ? 'display: none;' : ''; ?>">
			<label for="option-tooltip-<?php echo esc_attr( $x ); ?>"><?php echo esc_html__( 'Placeholder', 'yith-woocommerce-product-add-ons' ); ?>:</label>
				<div class="field">
					<input type="text" name="options[placeholder][]" id="option-tooltip-<?php echo esc_attr( $x ); ?>" value="<?php echo esc_html( $addon->get_option( 'placeholder', $x ) ); ?>">
				</div>
		</div>
			<!-- End option field -->
	</div>
