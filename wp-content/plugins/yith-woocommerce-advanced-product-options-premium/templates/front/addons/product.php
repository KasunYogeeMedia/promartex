...
if ( $_product instanceof WC_Product ) {
	...
	$_product_image = wp_get_attachment_image_src( get_post_thumbnail_id( $parent_id ? $parent_id : $product_id ), 'thumbnail' );
	...
	$selected = $addon->get_option( 'default', $x, 'no' ) === 'yes' ? 'selected' : '';
				...
				<?php if ( $_product->get_stock_status() === 'instock' ) : ?>
					...
					<div class="option-add-to-cart">
						<?php if ( $addon->get_setting( 'show_quantity' ) === 'yes' ) : ?>
							<input type="number" class="wapo-product-qty" data-product-id="<?php echo esc_attr( $_product->get_id() ); ?>" name="yith_wapo_product_qty[<?php echo esc_attr( $addon->id . '-' . $x ); ?>]" value="1" min="1" max="<?php echo esc_attr( $_product->get_stock_quantity() ); ?>" style="min-width: 50px; width: 5em; margin-right: 10px; float: left;">
						<?php endif; ?>
