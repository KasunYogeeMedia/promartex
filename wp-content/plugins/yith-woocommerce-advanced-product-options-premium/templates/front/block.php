...
<div id="yith-wapo-block-<?php echo esc_attr( $block->id ); ?>" class="<?php echo esc_attr( $block_classes ) ?>">
	...
	foreach ( $addons as $key => $addon ) :
		if ( '1' === $addon->visibility && yith_wapo_is_addon_type_available( $addon->type ) ) :
			...
			$addon_image_replacement       = $addon->get_setting( 'image_replacement', 'no' );
			$addon_options_images_position = $addon->get_setting( 'options_images_position', 'above' );
			...
			$options_width_css        = $show_in_a_grid ? 'width: ' . ( $options_width ) . '%;' : 'width: 100%;';
			...
			// Sell individually.
			$sell_individually = $addon->get_setting( 'sell_individually', 'no' );
			...
			$addon_classes       = apply_filters( 'yith_wapo_addon_classes',
				...
				esc_attr( $conditional_logic_class ) . ' ' . esc_attr( 'yes' === $sell_individually ? 'sell_individually': '' ), $addon );
	...
	?>
			...
			<div id="yith-wapo-addon-<?php echo esc_attr( $addon->id ); ?>"
				...
				<?php if ( '' !== $addon_title ) : ?>
					<<?php echo esc_attr( $style_addon_titles ); ?> class="wapo-addon-title <?php echo esc_attr( $toggle_status ); ?>"><?php echo esc_html( $addon_title ); ?>
				...
				if ( 'html_heading' === $addon->type || 'html_separator' === $addon->type || 'html_text' === $addon->type ) {
					...
					for ( $x = 0; $x < $options_total; $x++ ) {
						if ( file_exists( YITH_WAPO_DIR . '/templates/front/addons/' . $addon->type . '.php' ) ) {
							...
							$enabled = $addon->get_option( 'addon_enabled', $x, 'yes' );
