...
if ( ! class_exists( 'YITH_WAPO_Color_Label_Variations_Admin' ) ) {
	...
	class YITH_WAPO_Color_Label_Variations_Admin {
		...
		public function __construct() {
			...
			// Module option tab.
			add_filter( 'yith_wapo_admin_panel_tabs', array( $this, 'add_admin_tab' ), 10, 1 );
			add_filter( 'yith_plugin_panel_item_options_path', array( $this, 'add_options_array' ), 10, 3 );
		...
		/**
		 * Add module option tab to default plugins tab
		 *
		 * @since 2.0.0
		...
		public function add_admin_tab( $tabs ) {
			$tabs['color-label-variations'] = __( 'Color and Label Variations', 'yith-woocommerce-product-add-ons' );
			return $tabs;
		}

		/**
		 * Filter module options array path for module tab.
		 *
		 * @since 2.0.0
		 * @author Francesco Licandro
		 * @param string $path Current tab options path.
		 * @param string $options_path Base tab options path.
		 * @param string $item Current tab item.
		 */
		public function add_options_array( $path, $options_path, $item ) {
			if ( 'color-label-variations' === $item ) {
				$path = YITH_WAPO_WCCL_DIR . 'module-options/color-label-variations-options.php';
			}
		...
		public function enqueue_scripts() {
			...
			if (
				...
				wp_enqueue_style( 'yith-wccl-admin', YITH_WAPO_WCCL_ASSETS_URL . 'css/admin.css', array( 'wp-color-picker' ), YITH_WAPO_VERSION );
