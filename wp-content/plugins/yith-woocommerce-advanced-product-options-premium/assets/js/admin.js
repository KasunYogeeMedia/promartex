...
( function ( $ ) {
  ...
  $( '#addon_options' ).sortable({
    ...
    delay: 150,
    update: function( event, ui ) {
     /** Reorder name of "Selected by default" */
      $( '#addon_options .option' ).each( function( index, element ) {
           let selectByDefault = $( element ).find( '.selected-by-default input.selected-by-default-chbx' );
           selectByDefault.attr( 'name', 'options[default][' + index + ']' );
        }
      );
    }
