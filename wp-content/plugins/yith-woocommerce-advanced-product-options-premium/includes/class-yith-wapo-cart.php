							...
							if ( 'number' === $info['addon_type'] ) {
								if ( 'value_x_product' === $info['price_method'] ) {
									...
									if ( 'multiplied' === $info['price_type'] ) {
										$option_price      = $value * floatval( $info['price'] );
				...
				foreach ( $cart_item['yith_wapo_options'] as $index => $option ) {
					foreach ( $option as $key => $value ) {
						if ( $key && '' !== $value ) {
							...
							$sign = apply_filters( 'yith_wapo_price_sign', 'decrease' === $info['price_method'] ? '-' : '+' );
							...
							if ( in_array( $info['addon_type'], array(
								...
								if ( $option_product && $option_product instanceof WC_Product ) {
									$value = $option_product_qty . ' x ' . $option_product->get_formatted_name();
							...
							$value = apply_filters( 'yith_wapo_addon_value_on_cart', $value, $key );
