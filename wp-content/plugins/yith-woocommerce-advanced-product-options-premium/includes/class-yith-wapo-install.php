...
if ( ! class_exists( 'YITH_WAPO_Install' ) ) {
	...
	class YITH_WAPO_Install {
		...
		public function __construct() {
			...
			add_action( 'yith_wapo_run_update_callback', array( $this, 'run_update_callback' ) );
		}
		...
		public function check_version() {
			...
			if ( $this->needs_db_update() && 'old_panel' === $panel_version ) {
				$this->install();
		...
		public function needs_db_update() {
			...
			else if ( version_compare( $current_db_version, YITH_WAPO_DB_VERSION, '<' ) ){
				return true;
			}
			else{
				return false;
		...
		public function remove_del_column_from_db() {
			...
			if( $wpdb->get_var("SHOW TABLES LIKE '$blocks_table_name'") === $blocks_table_name) {
				...
				$check_deleted_column = $wpdb->get_results( $wpdb->prepare(
					"SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s AND COLUMN_NAME = %s ",
					DB_NAME, $blocks_table_name, $column_name
				) );
			...
			if( $wpdb->get_var("SHOW TABLES LIKE '$addons_table_name'") === $addons_table_name) {
				...
				$check_deleted_column = $wpdb->get_results( $wpdb->prepare(
					"SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s AND COLUMN_NAME = %s ",
					DB_NAME, $addons_table_name, $column_name
				) );
