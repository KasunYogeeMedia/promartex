...
if ( ! class_exists( 'YITH_WAPO' ) ) {
	...
	class YITH_WAPO {
		...
		public function load_modules() {
			...
			//todo:
			// remove yith_wapo_settings_disable_wccl option from the database
		...
		public function get_addon_types() {
			$addon_types = array(
				...
				array(
					'slug'  => 'select',
					'name'  => _x( 'Select', 'ADMIN: Select Item name', 'yith-woocommerce-product-add-ons' ),
					'label' => _x( 'Select item', 'ADMIN: Select Item label', 'yith-woocommerce-product-add-ons' ),
				),
				array(
					'slug'  => 'label',
					'name'  => __( 'Label or image', 'yith-woocommerce-product-add-ons' ),
					'label' => _x( 'Label or image', 'ADMIN: Label or image label', 'yith-woocommerce-product-add-ons' ),
				),
				array(
					'slug'  => 'product',
					'name'  => __( 'Product', 'yith-woocommerce-product-add-ons' ),
					'label' => _x( 'Product', 'ADMIN: Product label', 'yith-woocommerce-product-add-ons' ),
				),
				array(
					'slug'  => 'date',
					'name'  => __( 'Date', 'yith-woocommerce-product-add-ons' ),
					'label' => _x( 'Date', 'ADMIN: Date label', 'yith-woocommerce-product-add-ons' ),
				),
				array(
					'slug'  => 'file',
					'name'  => _x( 'File upload', 'ADMIN: File Upload label', 'yith-woocommerce-product-add-ons' ),
					'label' => _x( 'File uploader', 'ADMIN: File Upload label', 'yith-woocommerce-product-add-ons' ),
				),
				array(
					'slug'  => 'colorpicker',
					'name'  => __( 'Color picker', 'yith-woocommerce-product-add-ons' ),
					'label' => _x( 'Color Picker', 'ADMIN: Color picker label', 'yith-woocommerce-product-add-ons' ),
				),
			);
			return $addon_types;
		}
		...
		public function duplicate_block( $block_id ) {
			...
			if ( $block_id > 0 ) {
				...
				$query_addons       = "SELECT * FROM {$wpdb->prefix}yith_wapo_addons WHERE block_id='$block_id' ";
		...
		public function remove_block( $block_id ) {
			...
			if ( $block_id > 0 ) {
				...
				$wpdb->delete( $wpdb->prefix . 'yith_wapo_blocks' , array( 'id' => $block_id ) );
		...
		public function save_addon( $request, $method = '' ) {
			...
			if ( isset( $request['addon_id'] ) && isset( $request['block_id'] ) && $request['block_id'] > 0 ) {
				...
				$data = array(
					...
					'priority'   => $request['addon_priority'] ?? 0, // Priority set if comes from migration.
					'visibility' => 1,
				);
		...
		public function save_addon_enable_value_formatted( $request ) {
			...
			if ( ! in_array( $request['addon_type'], $excluded_addon_types, true ) ) {
				...
				for ( $i = 0; $i < $addons_count; $i ++ ) {
					$options['addon_enabled'][ $i ] = isset( $options['addon_enabled'][ $i ] ) && 'yes' === $options['addon_enabled'][ $i ] ? $options['addon_enabled'][ $i ] : 'no';
