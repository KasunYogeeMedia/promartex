...
class WPD_Design {
	...
	/**
	 * Add custom design to cart by ajax.
	 *
	 * @return void
	 */
	 ...
	 * Add design to cart.
	 *
	 * @param array $final_canvas_parts A canvas part data.
	 * @return bool|string
	 */
		...
		foreach ( $variations as $variation_name => $variation_info ) {
			...
			if ( 'simple' === $product->get_type() ) {
				...
				if ( method_exists( WC()->session, 'get' ) ) {
					$variation = get_transient( WC()->session->get( 'wpd_key' ) );
				}
			}
			...
			$newly_added_cart_item_key = $woocommerce->cart->add_to_cart( $product_id, $quantity, $variations_id, $variation, array( 'wpc_generated_data' => $final_canvas_parts ) );
	 ...
	 * Merge pictures.
	 *
	 * @param string $base_layer Base layer.
	 * @param string $top_layer Top layer.
	 * @param string $final_img Final image path.
	 ...
	 * @return void
	 */
	public function get_watermarked_preview() {
	...
	/**
	 * Save custom design for later.
	 *
	 * @return void
	 */
	public function save_custom_design_for_later_ajax() {
		$final_canvas_parts = $_POST['final_canvas_parts'];
			...
			foreach ( $data as $part => $part_data ) {
				...
				foreach ( $decoded_json->objects as $key => $object ) {
					...
					if ( $object_type == 'text' || $object_type == 'i-text' ) {
						$nb_chars        = strlen( $decoded_json->objects[ $key ]->text );
						$nb_lines        = substr_count( $decoded_json->objects[ $key ]->text, "\n" ) + substr_count( $decoded_json->objects[ $key ]->text, '|n' );
						...
						if ( ! $is_just_one_ine ) {
							array_push(
								$elements[ $object_type ],
								array(
									'txt_nb_chars' => ( $nb_chars - $nb_lines ),
									'txt_nb_lines' => ( $nb_lines + 1 ),
									'txt_nb'       => $totals_by_type[ $object_type ],
								)
							);
						} else {
							array_push(
								$elements[ $object_type ],
								array(
									'txt_nb_chars' => $nb_chars,
									'txt_nb_lines' => $nb_lines,
									'txt_nb'       => $totals_by_type[ $object_type ],
								)
							);
						}
				...
				if ( $object[ $param ] > $value ) {
					array_push(
						...
						array(
							...
							'nb_attr'            => $object[ $param ],
							'nb_additional_item' => $object[ $param ] - $value,
							'type'               => $type,
						)
					);
				} else {
	...
	private function get_group_results( $priceable_elements, $rules ) {
		...
		foreach ( $rules as $rule_arr ) {
			...
			elseif ( $this->wpc_starts_with( $rule_arr['param'], 'img' ) ) {
			...
			elseif ( $this->wpc_starts_with( $rule_arr['param'], 'path' ) ) {
			...
			elseif ( $this->wpc_starts_with( $rule_arr['param'], 'canvas' ) ) {
			...
			elseif ( $this->wpc_starts_with( $rule_arr['param'], 'part' ) ) {
	...
	private function save_pdf_output( $variation_id, $input_file, $output_file, $pdf_fonts = array() ) {
		...
		// if ($pdf_format == 'custom') {
			$pdf_width = get_proper_value( $variation_output_settings, 'output-w', 0 );
		$pdf_height    = get_proper_value( $variation_output_settings, 'output-h', 0 );
			$pdf_unit  = get_proper_value( $variation_output_settings, 'output-unit', 'in' );
		if ( $pdf_unit == 'px' ) {
			$pdf_unit   = 'mm';
		...
		if ( $ext == 'svg' ) {
			...
			$pf = TCPDF_STATIC::getPageSizeFromFormat( get_proper_value( $variation_output_settings, 'pdf-format', 'A0' ) );
	...
	/**
	 * Start woocommerce cookie session.
	 *
	 * @return void
	 */
