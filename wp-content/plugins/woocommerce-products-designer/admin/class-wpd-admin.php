...
class WPD_Admin {
	...
	public function enqueue_scripts() {
		if ( is_wpd_admin_screen() ) {
			...
			wp_localize_script( 'o-admin', 'home_url', array( 'ajax_url', home_url( '/' ) ) );
	...
	public function get_premium_features_page() {
		?>
	...
	public function get_v5_upgrader_buttons() {
		?>
	...
	public function get_facebook_redirection_url() {

		echo WPD_URL . 'includes/hybridauth/?hauth.done=Facebook';
	}

	public function get_instagram_redirection_url() {
	 ...
	 * @global array $wpd_settings
	 */
	public function notify_customization_page_missing() {
		global $wpd_settings;
	...
	public function get_help_notices() {
	...
	public function get_missing_parts_notice() {
		$screen = get_current_screen();
	 ...
	 */
	public function notify_minmimum_required_parameters() {
		global $wpd_settings;
	...
	public function wpc_add_custom_mime_types( $mimes ) {
		return array_merge(
			$mimes,
			array(
				'svg' => 'image/svg+xml',
	...
	public function wpd_get_subscription_notice() {
	...
	public function wpd_hide_notice() {
		set_transient( 'wpd-hide-notice', 'hide', 2 * WEEK_IN_SECONDS );
		echo 'ok';
		die();
	}

	public function run_wpc_db_updates_requirements() {
		$v7_upgrade = get_option( 'wpd-db-version' );
		if ( ( version_compare( $v7_upgrade, '7.0', '<' ) || $v7_upgrade == false )
		) {
			?>
		<div class="updated" id="wpc-updater-container">
			<strong><?php echo _e( 'Ouidah Product Designer database update required', 'wpd' ); ?></strong>
			<div>
			<?php
			echo _e(
				"Hi! This version of the Ouidah Product Designer made some changes in the way it's data are stored. <br>In order to work properly, we just need you to:"
				. '<ol>'
