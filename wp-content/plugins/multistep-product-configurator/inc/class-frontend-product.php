...
if(!class_exists('MSPC_Frontend_Product')) {
	...
	class MSPC_Frontend_Product {
		...
		public function __construct() {
			...
			add_shortcode( 'mspc', array( &$this, 'shortcode_mspc') );
		...
		public function add_mspc_form() {
			...
			if( $product && mspc_enabled( $product->get_id() ) && $product->has_attributes() ) {
				...
				$step_by_step = get_post_meta($product->get_id(), 'mspc_step_by_step', true); //step-by-step
				$step_by_step_class = $step_by_step == 'yes' ? ' mspc-step-by-step' : '';
				$tooltip_title = get_post_meta($product->get_id(), 'mspc_tooltip_title', true);
				$auto_scroll = get_post_meta($product->get_id(), 'mspc_auto_scroll', true); //auto-next
				$auto_scroll_class = $auto_scroll == 'yes' ? ' mspc-auto-scroll' : '';
				...
				?>
				<div class="mspc-wrapper mspc-clearfix mspc-module-<?php echo $module; ?><?php echo $auto_next_class; echo $step_by_step_class; echo $auto_scroll_class;?>">
			...
			foreach($terms as $term):
				...
				if( class_exists('FPD_Parameters') ) {
					...
					if( !empty($fpd_params) ) {
						...
						if( isset($fpd_params['enabled']) && $fpd_params['enabled'] && fpd_not_empty($fpd_params['enabled']) ) {
		...
		public function shortcode_mspc() {

			ob_start();

			echo $this->add_mspc_form();
