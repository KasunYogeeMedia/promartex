jQuery(document).ready(function() {
	...
	jQuery('.mspc-clear-selection').on('click', function(evt) {
	...
	$variationItems.on('click', function() {
		...
		var fpdProductLoading = typeof fpdWcLoadAjaxProduct === 'undefined' ? false : fpdWcLoadAjaxProduct;

		var $this = jQuery(this);

		if(!fpdProductLoading) {
		...
		if($wrapper.find('.mspc-accordion').size() > 0) {
			...
			if( !$this.hasClass('active') ) {
				...
				$this.next('.mspc-content:first').delay(time).slideDown(time, function() {
					...
					if($wrapper.hasClass('mspc-auto-scroll') && noInitialScroll) {
	...
	function _addElementToFpd(variation) {
		...
		var $variation = jQuery(variation);
